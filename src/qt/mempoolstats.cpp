...
{
    printf("is visible %d is client model %d\n", isVisible(), (m_clientmodel ? 1 : 0));
    if (!m_clientmodel)
        return;

    int64_t timestart = GetTimeMicros();
    printf("Draw image start %lld\n", timestart);
    m_scene->clear();
//    m_scene->addText(tr("Mempool Statistics"));
...
//        FILE *filestr = fsbridge::fopen("/Users/jonasschnelli/Desktop/statsdump_2_long", "rb");
...
//        file.fclose();
        ...
        const int amountOfLinesH = 5;
        if (max_txcount > 0) {
            int val = floor(log10(1.0*max_txcount/amountOfLinesH));
            int stepbase = pow(10.0f, val);
            int step = ceil((1.0*max_txcount/amountOfLinesH) / stepbase) * stepbase;
            max_txcount_graph = step*amountOfLinesH;
        }
        ...
        {
            qreal lY = bottom-i*(maxheight_g/(amountOfLinesH-1));
            tx_count_grid_path.moveTo(GRAPH_PADDING_LEFT, lY);
            tx_count_grid_path.lineTo(GRAPH_PADDING_LEFT+maxwidth, lY);

            size_t grid_tx_count = (float)i*(max_txcount_graph-bottomTxCount)/(amountOfLinesH-1) + bottomTxCount;
        ...
        for (const interfaces::mempool_feeinfo& list_entry : m_clientmodel->m_mempool_feehist[0].second) {
            ...
            QColor brush_color = colors[(i < colors.size() ? i : colors.size()-1)];
            ...
            if (i+1 == m_clientmodel->m_mempool_feehist[0].second.size()) {
    ...
    printf("Draw image path created, time: %lld\n", GetTimeMicros()-timestart);
    int i = 0;
    QString total_text = "Total txn "+QString::number(max_txcount);
    for (auto feepath : fee_paths) {
        ...
        QColor pen_color = colors[(i < colors.size() ? i : colors.size()-1)];
        ...
        if (m_selected_range >= 0 && m_selected_range == i) {
            total_text = "txn in range: "+QString::number(fee_subtotal_txcount[i]);
        }
    ...
    printf("Draw image end, time: %lld\n", GetTimeMicros()-timestart);
}
