...
static bool FindBlockPos(FlatFilePos &pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown = false)
{
    ...
    if (!fKnown) {
        while (vinfoBlockFile[nFile].nSize + nAddSize >= (gArgs.GetBoolArg("-smallblockfiles", false) ? 0x10000 /* 64kb */ : MAX_BLOCKFILE_SIZE)) {
...
static FlatFileSeq BlockFileSeq()
{
    return FlatFileSeq(GetBlocksDir(), "blk", gArgs.GetBoolArg("-smallblockfiles", false) ? 0x4000 /* 16kb */ : BLOCKFILE_CHUNK_SIZE);
}
