    ...
    while (msg_bytes.size() > 0) {
        ...
        if (deserializer->Complete()) {
            ...
            if (result) {
                ...
                assert(result->m_raw_message_size <= msg_bytes.size());
...
FUZZ_TARGET_INIT(p2p_transport_deserializer, initialize_p2p_transport_deserializer)
{
    ...
    test_deserializer(v1_deserializer, msg_bytes, CMessageHeader::HEADER_SIZE);
    ...
    test_deserializer(v2_deserializer, msg_bytes, CHACHA20_POLY1305_AEAD_AAD_LEN + CHACHA20_POLY1305_AEAD_TAG_LEN);
}
