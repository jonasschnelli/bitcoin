...
class FeatureBlockfilterindexPruneTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-smallblockfiles", "-prune=1"], ["-smallblockfiles", "-prune=1", "-blockfilterindex=1"]]

    def run_test(self):
        # test basic pruning compatibility & filter access of pruned blocks
        self.log.info("check if we can access a blockfilter when pruning is enabled but no acctual pruned blocks")
        ...
        self.stop_node(1)
        self.start_node(1, extra_args=["-smallblockfiles", "-prune=1"])
