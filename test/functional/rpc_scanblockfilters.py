...
class scanblocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert(blockhash in self.nodes[0].scanblocks(["addr("+addr_1+")"]))
        ...
        assert(blockhash not in self.nodes[0].scanblocks(["addr("+addr_1+")"], self.nodes[0].getblockheader(blockhash_new)['height']))
